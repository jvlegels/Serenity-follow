...
const rootElement: HTMLDivElement = appRoot;
...
function shellTemplate(route: Route): string {
  return `
    <header class="topbar">
      <a class="brand" href="#/">
...
function pageTemplate(route: Route): string {
  if (route.name === 'history') return historyTemplate();
  if (route.name === 'settings') return settingsTemplate();
  if (route.name === 'signal') return signalDetailTemplate(findRecord(route.id));
...
function historyTemplate(): string {
  return `
...
function compactSignalTemplate(record: SignalRecord): string {
  return `
...
function postCardTemplate(record: SignalRecord): string {
  return `
...
function confidencePanel(record: SignalRecord | null): string {
...
function routeFromHash(): Route {
  ...
  if (signalMatch?.[1]) return { name: 'signal', id: signalMatch[1] };
  return { name: 'dashboard' };
}

function findRecord(signalId: string): SignalRecord | null {
...
function evidenceChip(item: { label: string; impact: 'positive' | 'negative' | 'neutral' }): string {
...
function setLoading(userInitiated: boolean): void {
...
function statusLabel(mode: AdviceResponse['sourceStatus']['mode']): string {
  return mode === 'live' ? 'Live X' : mode === 'mock' ? 'Mock data' : 'Needs attention';
}

function levelLabel(level: SignalRecord['analysis']['level']): string {
  return level === 'high' ? 'Potential signal' : level === 'possible' ? 'Possible signal' : 'No signal';
}

function widgetTone(widget: WidgetState): string {
  return widget.color === 'green' ? 'hero-green' : widget.color === 'amber' ? 'hero-amber' : 'hero-grey';
}

function widgetColorLabel(color: WidgetState['color']): string {
